...
def make_model():
    ...
    def loglike(value=1.0, fg=fg, gamma=gamma):
        f = fg[0]
        g = fg[1:]
        return gamma*f + gamma*2.0*(min(0., g[0]) + min(0., g[1]) + min(0., g[2]) +  min(0., g[3]))
        # return gamma * f + \
...
if __name__ == '__main__':
    ...
    smc = SteadyPaceSMC(mcmc, num_particles=2000, num_mcmc=5, verbose=4,
                 gamma_is_an_exponent=True,
                 ess_reduction=0.9, adapt_proposal_step=True,
                 mpi=mpi)
    smc.initialize(0.1)
    results = []
    for gamma in np.linspace(0., 50, 100)[1:]:
        smc.move_to(gamma)
